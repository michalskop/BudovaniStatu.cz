...
var BubbleTree = function(config, onHover, onUnHover) {
	...
	//**ms
	//var to speed-up diacritics removal
	...
	me.traverse = function(node, index) {
		...
		if (!node.color) {
			...
			//**ms
			if (node.level == 0) node.color = '#999999'; 
			else if (node.level > 1) node.color = node.parent.color;
			else { 
			  node.color = '#' + Math.floor(Math.random()*10) + Math.floor(Math.random()*10) + Math.floor(Math.random()*10) + Math.floor(Math.random()*10) + Math.floor(Math.random()*10) + Math.floor(Math.random()*10);
			  //node.color = '#999999'; 
			}
		}
		...
		while (me.nodesByUrlToken.hasOwnProperty(node.urlToken)) {
			...
			//**ms:
			/*if (i>0) {
			  var tmp = node.urlToken.split('-');
			  tmp = tmp.pop();
			  //alert (node.urlToken);
			  if (IsNumeric(tmp)) {
			    node.urlToken = node.urlToken.substr(0,node.urlToken.length - tmp.length) + i;
			    //alert(node.urlToken); 
			  } else node.urlToken += '-' + i;
			}
			i++;*/
		} 
...
/**
* //**ms:
* more functions, shoule be moved outside bubbletree
*/
...
function getHistoricalValues(node) {
	...
	//before ajax call
	$("#bs-table-ajax-loader").html('<img src="sites/all/modules/bubbletree/ajax-loader.gif" alt="Wait" /> Nahrávám data a generuji tabulku, bude to jen chvilka ...');
	$("#bs-table").html('');
...
var sort_by = function(field, reverse, primer){
   ...
   return function (a,b) {
       ...
       else if (A > B) return -1* [-1,1][+!!reverse]
       else return 0;                 
   }
}
